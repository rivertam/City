...
export default class TensorFieldGUI extends TensorField {
  ...
  private getCrossLocations(): Vector[] {
    ...
    const originX =
      diameter * Math.floor(this.domainController.origin.x / diameter);
    const originY =
      diameter * Math.floor(this.domainController.origin.y / diameter);
    ...
    if (this.drawCentre) {
      ...
      this.getBasisFields().forEach((field) =>
        field.FIELD_TYPE === FIELD_TYPE.Grid
          ? canvas.drawSquare(
              this.domainController.worldToScreen(field.centre),
              7,
            )
          : canvas.drawCircle(
              this.domainController.worldToScreen(field.centre),
  ...
  protected addField(field: BasisField): void {
    ...
    const removeFieldObj = {
      remove: () => this.removeFieldGUI(field, deregisterDrag),
    };
