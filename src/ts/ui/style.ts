...
export class DefaultStyle extends Style {
  ...
  public draw(canvas = this.canvas as DefaultCanvasWrapper): void {
    ...
    if (this.colourScheme.zoomBuildings) {
      bgColour =
        this.domainController.zoom >= 2
          ? this.colourScheme.bgColourIn
          : this.colourScheme.bgColour;
    } else {
    ...
    if (this.heightmap) {
      ...
      // Pseudo-3D
      if (
        this.colourScheme.buildingModels &&
        (!this.colourScheme.zoomBuildings || this.domainController.zoom >= 2.5)
      ) {
...
export class RoughStyle extends Style {
  ...
  public update() {
    const dragging =
      this.dragController.isDragging || this.domainController.isScrolling;
