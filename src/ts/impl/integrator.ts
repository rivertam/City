...
export class EulerIntegrator extends FieldIntegrator {
  ...
  integrate(point: Vector, major: boolean): Vector {
    return this.sampleFieldVector(point, major).multiplyScalar(
      this.params.dstep,
    );
  }
}
...
export class RK4Integrator extends FieldIntegrator {
  ...
  integrate(point: Vector, major: boolean): Vector {
    ...
    const k23 = this.sampleFieldVector(
      point.clone().add(Vector.fromScalar(this.params.dstep / 2)),
      major,
    );
    const k4 = this.sampleFieldVector(
      point.clone().add(Vector.fromScalar(this.params.dstep)),
      major,
    );
