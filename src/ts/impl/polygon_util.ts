...
export default class PolygonUtil {
  ...
  public static subdividePolygon(p: Vector[], minArea: number): Vector[][] {
    ...
    const bisect = [
      averagePoint.clone().add(perpVector),
      averagePoint.clone().sub(perpVector),
    ];
  ...
  public static resizeGeometry(
    ...
    try {
      const jstsGeometry = isPolygon
        ? PolygonUtil.polygonToJts(geometry)
        : PolygonUtil.lineToJts(geometry);
