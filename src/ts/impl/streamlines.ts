...
export default class StreamlineGenerator {
  ...
  getBestNextPoint(
    ...
    for (const sample of nearbyPoints) {
      if (!sample.equals(point) && !sample.equals(previousPoint)) {
        ...
        // Filter by angle
        if (
          angleBetween < this.params.joinangle &&
          distanceToSample < closestDistance
        ) {
    ...
    if (closestSample !== null) {
      closestSample = closestSample
        .clone()
        .add(direction.setLength(this.params.simplifyTolerance * 4));
    }
  ...
  protected doesStreamlineCollideSelf(
    ...
    if (streamlineForwards.length > this.nStreamlineLookBack) {
      ...
      for (
        let i = 0;
        i < streamlineForwards.length - this.nStreamlineLookBack;
        i += this.nStreamlineStep
      ) {
        if (
          testSample.distanceToSquared(streamlineForwards[i]) <
          this.dcollideselfSq
        ) {
          return true;
        }
      ...
      for (
        let i = 0;
        i < streamlineBackwards.length;
        i += this.nStreamlineStep
      ) {
        if (
          testSample.distanceToSquared(streamlineBackwards[i]) <
          this.dcollideselfSq
        ) {
          return true;
        }
  ...
  protected integrateStreamline(seed: Vector, major: boolean): Vector[] {
    ...
    while (
      count < this.params.pathIterations &&
      (forwardParams.valid || backwardParams.valid)
    ) {
      ...
      if (
        !pointsEscaped &&
        sqDistanceBetweenPoints > this.paramsSq.dcirclejoin
      ) {
        pointsEscaped = true;
      }
